...
from RaspPiReader.ui.program_selection_form import Ui_ProgramSelectionForm  # generated from program_selection_form.ui
...
class ProgramSelectionFormHandler(QtWidgets.QWidget):
    ...
    def update_program_info(self, index):
        ...
        if default_program:
            ...
            info_text = "No default program found for this selection."
        # Update the UI label if it exists; otherwise print to console.
    ...
    def start_cycle(self):
        ...
        if not default_program:
            QtWidgets.QMessageBox.warning(
                self, "Warning", f"No preset program found for Program {program_index}"
            )
            return

        # Create a CycleData record. For numeric fields, if the default is empty, use 0.0.
        try:
            cycle = CycleData(
                ...
                if default_program.cool_down_temp and default_program.cool_down_temp.strip()
                else 0.0,
                core_temp_setpoint=float(default_program.core_temp_setpoint)
                if default_program.core_temp_setpoint and default_program.core_temp_setpoint.strip()
                else 0.0,
                temp_ramp=float(default_program.temp_ramp)
                if default_program.temp_ramp and default_program.temp_ramp.strip()
                else 0.0,
                set_pressure=float(default_program.set_pressure)
                if default_program.set_pressure and default_program.set_pressure.strip()
                else 0.0,
                maintain_vacuum=float(default_program.maintain_vacuum)
                if default_program.maintain_vacuum and default_program.maintain_vacuum.strip()
                else 0.0,
                initial_set_cure_temp=float(default_program.initial_set_cure_temp)
                if default_program.initial_set_cure_temp and default_program.initial_set_cure_temp.strip()
                else 0.0,
                final_set_cure_temp=float(default_program.final_set_cure_temp)
                if default_program.final_set_cure_temp and default_program.final_set_cure_temp.strip()
                else 0.0,
            )
            ...
            QtWidgets.QMessageBox.critical(
                self, "Database Error", f"Could not save cycle data: {e}"
            )
            return
